...
class DirectionLevelThree: UIViewController {
    ...
    @objc func diagonalDetection(sender: UIPanGestureRecognizer) {
    ...
    @IBAction func stopRecord(_ sender: UIButton) {
        ...
        if bestString.lowercased().contains("stop"){
            ...
            let utterance = AVSpeechUtterance(string: "Thank you, you tapped on " + String(self.swipeCount) + "out of " + String(self.numOfSwipes) + "bars")
            utterance.voice = AVSpeechSynthesisVoice(language: "en-US")
            utterance.rate = 0.5
            let synthesizer = AVSpeechSynthesizer()
            synthesizer.speak(utterance)
