...
class DirectionLevelOne: UIViewController {
    ...
    func beginDirectionDetection() {
        ...
        let rightSwipe = UISwipeGestureRecognizer(target: self, action: #selector(rightSwipeDetection(sender:)))
        // right by default
        
        let leftSwipe = UISwipeGestureRecognizer(target: self, action: #selector(leftSwipeDetection(sender:)))
        leftSwipe.direction = .left
        
        let upSwipe = UISwipeGestureRecognizer(target: self, action: #selector(upSwipeDetection(sender:)))
        upSwipe.direction = .up
        
        let downSwipe = UISwipeGestureRecognizer(target: self, action: #selector(downSwipeDetection(sender:)))
        downSwipe.direction = .down
        
        let diagonalSwipe = UIPanGestureRecognizer(target: self, action: #selector(diagonalDetection(sender:)))
        diagonalSwipe.require(toFail: rightSwipe)
        diagonalSwipe.require(toFail: leftSwipe)
        diagonalSwipe.require(toFail: upSwipe)
        diagonalSwipe.require(toFail: downSwipe)
        
        PlayingView.addGestureRecognizer(diagonalSwipe)
        
        PlayingView.addGestureRecognizer(rightSwipe)
        PlayingView.addGestureRecognizer(leftSwipe)
        PlayingView.addGestureRecognizer(upSwipe)
        PlayingView.addGestureRecognizer(downSwipe)
    ...
    @objc func diagonalDetection(sender: UISwipeGestureRecognizer) {
        ...
        if sender.state == .began {
            ...
            if beginPoint.x - endPoint.x < 0 && beginPoint.y - endPoint.y > 0 && currentSwipe == 5{
                swipeCount += 1
                currentStatus = "yes"
                goodjob()
                stopNewSquareTest()
                prepareNew()
            }else if beginPoint.x - endPoint.x > 0 && beginPoint.y - endPoint.y > 0 && currentSwipe == 6{
                swipeCount += 1
                currentStatus = "yes"
                goodjob()
                stopNewSquareTest()
                prepareNew()
            }else if beginPoint.x - endPoint.x > 0 && beginPoint.y - endPoint.y < 0 && currentSwipe == 7{
                swipeCount += 1
                currentStatus = "yes"
                goodjob()
                stopNewSquareTest()
                prepareNew()
            }else if beginPoint.x - endPoint.x < 0 && beginPoint.y - endPoint.y < 0 && currentSwipe == 8{
                swipeCount += 1
                currentStatus = "yes"
                goodjob()
                stopNewSquareTest()
    ...
    @IBAction func bottomLeftUpperRightAnimate() {
        let originalTransform = whiteSquare.transform
        let TranslatedTransform = originalTransform.translatedBy(x: 0, y: -UIScreen.main.bounds.size.height*0.5)
        UIView.animate(withDuration: 1.5, animations: {
            self.whiteSquare.transform = TranslatedTransform
        })
